...
import { ScoutTask } from "../../../db/storage";
import { 
  Search, Eye, Shield, Users, Calendar, Sparkles, UserPlus, 
  Trash2, Compass, AlertCircle, BarChart2, Star, CheckCircle
} from "lucide-react";
...
export default function Scouting() {
  ...
  // State for active tabs: "Scouting Hub" | "Shortlist" | "Completed Reports"
  const [activeTab, setActiveTab] = useState<"hub" | "shortlist" | "completed">("hub");
  ...
  // Predefined scouts for our club
  const scoutsList: Scout[] = useMemo(() => {
    const tasks = activeSave.scoutingTasks.filter(t => !t.completed);
    return [
      { 
        id: "scout_1", 
        name: "Gary Cahill", 
        specialty: "Defenders & Physicality", 
        rating: 4,
        status: tasks.some(t => t.scoutId === "scout_1") ? "Scouting" : "Idle" 
      },
      { 
        id: "scout_2", 
        name: "Jean-Pierre", 
        specialty: "Technical Playmakers", 
        rating: 5,
        status: tasks.some(t => t.scoutId === "scout_2") ? "Scouting" : "Idle"
      },
      { 
        id: "scout_3", 
        name: "Jürgen Becker", 
        specialty: "Youth Prospects & Speed", 
        rating: 4,
        status: tasks.some(t => t.scoutId === "scout_3") ? "Scouting" : "Idle"
      }
    ];
  ...
  const assignScout = (player: Player, scoutId: string) => {
    ...
    const newTask: ScoutTask = {
      ...
      playerRating: player.potential,
      position: player.position,
      scoutId,
      daysRemaining: 2, // takes 2 matchday ticks
      completed: false
    };
    ...
    newState.gameLog.unshift(`Assigned scout to analyze ${player.name} (${player.position}). Report ready in 2 Matchdays.`);
    
    updateActiveSave(newState);
    setSearchQuery("");
    setScoutingResults([]);
    alert(`Assigned scout to analyze ${player.name}.`);
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Delegate scouts to analyze specific players and identify future transfer targets.
          </p>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-800 text-xs font-bold text-slate-400">
        <button
          ...
          className={`px-4 py-3 border-b-2 transition ${activeTab === "hub" ? "border-green-500 text-slate-100" : "border-transparent hover:text-slate-200"}`}
        >
          Scouting Hub
        </button>
        <button
          onClick={() => setActiveTab("shortlist")}
          className={`px-4 py-3 border-b-2 transition ${activeTab === "shortlist" ? "border-green-500 text-slate-100" : "border-transparent hover:text-slate-200"}`}
        >
          Shortlisted Targets ({shortlistedPlayers.length})
        </button>
        <button
          onClick={() => setActiveTab("completed")}
          className={`px-4 py-3 border-b-2 transition ${activeTab === "completed" ? "border-green-500 text-slate-100" : "border-transparent hover:text-slate-200"}`}
        >
          Scout Reports ({completedReports.length})
        </button>
      </div>
                  ...
                  completedReports.map((report) => (
                    ...
                    >
                      <div className="flex justify-between">
                        ...
                        <span className="w-7 h-7 rounded-lg bg-green-500/10 border border-green-500/20 text-green-500 flex items-center justify-center font-bold">
                          {report.position}
                        </span>
                      </div>
                      ...
                      <div className="border-t border-slate-900 pt-3 flex flex-col gap-2">
                        <div className="flex justify-between">
                          <span className="text-slate-500">True Potential:</span>
                          <span className="font-black text-green-400">{report.playerRating} / 99</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-slate-500">Est Value:</span>
                          <span className="font-extrabold text-slate-350">€{(calculateValue(report.playerRating, 22)/1000000).toFixed(1)}M</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-850 flex items-center gap-2 text-[10px] leading-relaxed text-slate-400 mt-1">
                        ...
                        <span>Recommended target. Has high developmental headroom and strong squad compatibility.</span>
                      </div>
                    </div>
                  ))
                )}
